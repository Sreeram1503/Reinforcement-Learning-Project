...
class kartEnv(Env):
    ...
    def step(self, action):
        ...
        # **New Penalty for Collisions**
        if self.state.players[0].kart.collision:  # Replace with actual collision detection
            step_reward -= 1.0  # Penalty for hitting an obstacle
        terminated = False
        truncated = False
    ...
    def render(self, mode='human', **kwargs):
        """
        Render the environment visually. If `mode` is 'human', display the current frame.
        """
        if mode == 'human':
            import matplotlib.pyplot as plt
            plt.imshow(self.pytux.k.render_data[0].image)
            plt.axis('off')
